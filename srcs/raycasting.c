...
double		hor_detection(t_wolf *wolf, double angle)
{
	t_fvct2		ratio;
	t_fvct2		inter;
	t_fvct2		dist;
	...
	ratio.x = ratio.y / tan(angle * PI180);
	...
	if (ret == 1)
	{
		...
		ret = sqrt(dist.x * dist.x + dist.y * dist.y) * cos((wolf->rot - angle) * PI180);
	}
	return (ret);
}

double		ver_detection(t_wolf *wolf, double angle)
{
	t_fvct2		ratio;
	t_fvct2		inter;
	t_fvct2		dist;
	...
	ratio.y = ratio.x / tan((90 - angle) * PI180);
...
void		draw_column(t_wolf *wolf, double dist, int num)
{
	double	column_size;
	int		i;
	int		iprint;
	...
	i = 0;
	imax = ((double)wolf->sdl.size.y - column_size) / 2.0;
	iprint = num;
	while (i < imax)
	{
		wolf->sdl.screen[iprint] = color_rgb(60, 60, 60);
		i++;
		iprint += wolf->sdl.size.x;
	}
	imax = (double)i + column_size;
	while (i < imax - 1)
	{
		wolf->sdl.screen[iprint] = color_rgb(6, 20, 150);
		i++;
		iprint += wolf->sdl.size.x;
	}
	while (i < wolf->sdl.size.y)
	{
		wolf->sdl.screen[iprint] = color_rgb(115, 115, 115);
		i++;
		iprint += wolf->sdl.size.x;
	}
}
...
void		raycasting(t_wolf *wolf)
{
	...
	iangle = wolf->fov / (double)wolf->sdl.size.x;
